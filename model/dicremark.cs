...
{
    public partial class dicremark : Form
    {
        public dicremark()
        {
            InitializeComponent();
        ...
        private void dicremark_Load(object sender, EventArgs e)
        {
            ...
            string jsonfile = Client_Conn.Client_type_Select("1", "20000", null);
            ...
            dataGridView1.Columns.Add(column_add("文件标签"));
            pklist.Clear();
            PageSorterbiaoqian();//分页
            AddBtndgv2(3);
        ...
        /// <param name="e"></param>
        public static string sent_apendix_name;
        public static string sent_appendix_id;
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dataGridView1.Columns[e.ColumnIndex].Name == "edit" && e.RowIndex >= 0)
            {
                ...
                sent_apendix_name = dataGridView1.Rows[e.RowIndex].Cells[1].Value.ToString();
                sent_appendix_id = pklist[e.RowIndex];

                EditAppendixType edit_type = new EditAppendixType();
                edit_type.ShowDialog();

                if (EditAppendixType.dialog_result == 1)
                {
                    dataGridView1.Rows.Clear();
                    string sea = btnsearch.Text;
                    string jsonfile = Client_Conn.Client_type_Select("1", "20000", sea);
                    JObject resultresponse = JObject.Parse(jsonfile);
                    string data = resultresponse["data"].ToString();
                    JObject datainfo = JObject.Parse(data);
                    JArray listinfo = JArray.Parse(datainfo["list"].ToString());
            ...
            if (dataGridView1.Columns[e.ColumnIndex].Name == "del" && e.RowIndex >= 0)
            {
                ...
                if (MessageBox.Show("确定删除？", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    //dataGridView1.Rows.Clear();
                    string a = Client_Conn.Client_fileremark_delet(b);
                    pklist.Clear();
                    //DataGridView dg = new DataGridView();    //重新绑定
                    //dataGridView1.Refresh();
                    dataGridView1.Rows.Clear();
                    string sea = btnsearch.Text;
                    string jsonfile = Client_Conn.Client_type_Select("1", "2000", sea);
                    JObject resultresponse = JObject.Parse(jsonfile);
                    string data = resultresponse["data"].ToString();
                    JObject datainfo = JObject.Parse(data);
                    JArray listinfo = JArray.Parse(datainfo["list"].ToString());
        ...
        public void PageSorterbiaoqian()
        {
        ...
        {
            AddAppendixType frm1 = new AddAppendixType();
            frm1.ShowDialog();
            //AddAppendixType frm1 = new AddAppendixType();
            if (AddAppendixType.DialogResult_1 == 1)
            {
                dataGridView1.Rows.Clear();
                string sea = btnsearch.Text;
                string jsonfile = Client_Conn.Client_type_Select("1", "20000", sea);
        ...
        public static string sent_name;
        public static string snet_id;
